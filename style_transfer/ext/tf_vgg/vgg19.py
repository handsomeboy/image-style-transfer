...
class Vgg19:
    ...
    def toBGR(self, rgb):
      rgb_scaled = rgb * 255
      red, green, blue = np.split(axis=3, indices_or_sections=3, 
                                  ary=rgb_scaled)
      bgr = np.concatenate((
          blue - VGG_MEAN[0],
          green - VGG_MEAN[1],
    ...
    def toRGB(self, bgr):
      blue, green, red = np.split(axis=3, indices_or_sections=3, 
                                  ary=bgr)
      rgb = np.concatenate((
        (red + VGG_MEAN[2]) / 255.0,
        (green + VGG_MEAN[1]) / 255.0,
        (blue + VGG_MEAN[0]) / 255.0
      ), axis=3)
